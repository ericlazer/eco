...
├── data/
│   └── partners.json            # All partner data
...
The main page (`ecosystem/index.html`) loads styles from `../styles/nexio.css` and logic from `../scripts/ecosystem.js` to display partner cards. The partner data is stored in `data/partners.json`.

### Step 3: Guide Generator Script
Run `node generate-guides.js` to generate the 24 guide pages in `ecosystem/[partner]/guide/`. The script reads `data/partners.json` and outputs a full HTML page for each partner.
...
## 🎨 Customization
Edit the colors or layout in `styles/nexio.css`. Add or update partner entries in `data/partners.json` and run the generator again to update the guides.
